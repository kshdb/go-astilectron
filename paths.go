...
func AstilectronDownloadSrc(versionAstilectron string) string {
	return fmt.Sprintf("https://electron.t.cnwtn.com/asticode/astilectron/archive/v%s.zip", versionAstilectron)
}
...
func ElectronDownloadSrc(os, arch, versionElectron string) string {
	...
	//electron代理地址
	// Return url
	//return fmt.Sprintf("https://electron.t.cnwtn.com/electron/releases/download/v%s/electron-v%s-%s-%s.zip", versionElectron, versionElectron, o, a)
	return fmt.Sprintf("https://registry.npmmirror.com/-/binary/electron/v%s/electron-v%s-%s-%s.zip", versionElectron, versionElectron, o, a)
}
